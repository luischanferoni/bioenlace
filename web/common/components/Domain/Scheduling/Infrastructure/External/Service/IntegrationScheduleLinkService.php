...
use common\components\Domain\Scheduling\Infrastructure\External\Service\FhirScheduleActorExtractor;
use common\components\Domain\Scheduling\Infrastructure\External\Service\FhirSchedulePesResolver;
use common\components\Domain\Scheduling\Infrastructure\External\Service\ScheduleActorSet;
...
final class IntegrationScheduleLinkService
{
    ...
    {
        $schedules = \common\components\Domain\Scheduling\Infrastructure\External\Service\FhirBundleHelper::collectResources($scheduleBundle, 'Schedule');
        $schedule = $schedules[0] ?? null;
        if ($schedule === null) {
            throw new \InvalidArgumentException('Bundle sin Schedule.');
        }

        $scheduleId = \common\components\Domain\Scheduling\Infrastructure\External\Service\FhirBundleHelper::resourceId($schedule);
