...
| Estado | Planificado — Fase 1 pendiente |
...
| `frontend/modules/api/v1/controllers/` | 19 archivos bajo `clinical/`; **41 planos** (`TurnosController`, `ServiciosController`, `PersonaController`, …) |
...
Seguir `turnos` no: su controller es plano, su UI JSON está bajo `scheduling/`, su URL es `/api/v1/turnos/…`. El puente entre esas tres formas es un mapa PHP escrito a mano.
