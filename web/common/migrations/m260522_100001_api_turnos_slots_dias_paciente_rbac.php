...
/**
 * RBAC: UI JSON elegir día antes de horarios (autogestión paciente).
 */
class m260522_100001_api_turnos_slots_dias_paciente_rbac extends Migration
{
    private const ROUTE_TYPE = 3;

    private const NEW_ROUTE = '/api/turnos/slots-dias-disponibles-como-paciente';

    private const PARENT_ROUTE = '/api/turnos/slots-disponibles-como-paciente';

    public function safeUp()
    {
        if (!in_array($this->db->driverName, ['mysql', 'mysqli'], true)) {
            echo "m260522_100001: omitido (driver {$this->db->driverName}).\n";
        ...
        if ($this->db->schema->getTableSchema($authItem, true) === null) {
            echo "m260522_100001: sin tabla auth_item, omitido.\n";
    ...
    private function ensureRoute(string $authItem, string $name, int $now): void
    {
        ...
        $this->db->createCommand()->insert($authItem, [
            ...
            'description' => 'API turnos: elegir día con slots (paciente)',
            'rule_name' => null,
            'data' => null,
            'created_at' => $now,
            'updated_at' => $now,
