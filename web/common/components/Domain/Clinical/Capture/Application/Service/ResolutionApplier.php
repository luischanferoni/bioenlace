 ...
 *
 * @see CaptureIssueFactory
 * @see ClinicalCaptureResolution
 */
final class ResolutionApplier
{
    private RowContractRegistry $rowContracts;

    public function __construct(?RowContractRegistry $rowContracts = null)
    {
        $this->rowContracts = $rowContracts ?? new YiiModelRowContractRegistry();
    }
    ...
    public function remapCompletenessToOriginalIndices(array $completeness, array $indexMap): array
    {
        ...
        foreach ($completeness['issues'] ?? [] as $issue) {
            ...
            $id = (string) ($issue['id'] ?? '');
            $parsed = CaptureIssueFactory::parseIssueId($id);
            if ($parsed !== null) {
                $cat = $parsed['category'];
                $newIdx = $parsed['index'];
                if (isset($indexMap[$cat][$newIdx])) {
                    $orig = $indexMap[$cat][$newIdx];
                    $issue['id'] = CaptureIssueFactory::issueId(
                        $cat,
                        $orig,
                        $parsed['field']
                    );
