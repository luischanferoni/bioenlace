 ...
 */
final class CaptureAnalysisCache
{
    ...
    public static function store(array $body, array $datosExtraidos, string $textoClinico): ?string
    {
        ...
        try {
            ...
            Yii::warning('CaptureAnalysisCache::store cache: ' . $e->getMessage(), 'encounter-doc');
        }
    ...
    public static function recallWithMeta(array $body, ?string $textoClinico = null): array
    {
        ...
        foreach ($tokens as $token) {
            try {
                ...
                $payload = null;
                Yii::warning('CaptureAnalysisCache::recall cache: ' . $e->getMessage(), 'encounter-doc');
            }
            if (is_array($payload)) {
    ...
    private static function storeDb(string $token, array $body, string $textoClinico, array $datosExtraidos): void
    {
        try {
            ...
            if (!$row->save()) {
                Yii::warning(
                    'CaptureAnalysisCache::storeDb: ' . json_encode($row->getErrors()),
                    'encounter-doc'
                );
            }
        } catch (\Throwable $e) {
            Yii::warning('CaptureAnalysisCache::storeDb: ' . $e->getMessage(), 'encounter-doc');
        }
    }
    ...
    private static function findDbByToken(string $token): ?EncounterCaptureAnalysis
    {
        try {
            ...
            Yii::warning('CaptureAnalysisCache::findDbByToken: ' . $e->getMessage(), 'encounter-doc');
    ...
    private static function findDbByContext(array $body, ?string $textoClinico): ?EncounterCaptureAnalysis
    {
        try {
            ...
            Yii::warning('CaptureAnalysisCache::findDbByContext: ' . $e->getMessage(), 'encounter-doc');
