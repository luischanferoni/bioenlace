 ...
 */
final class AssertConsultaAsyncSolicitudAllowed
{
    /** @var list<string> */
    ...
    public function assertPuedeCrear(int $idPersona, array $draft): void
    {
        ...
        $necesidad = trim((string) ($draft[SubmitConsultasSeguimientoIntake::DRAFT_SEGUIMIENTO_NECESIDAD] ?? ''));
    ...
    private function assertNoMedicacionAbierta(int $idPersona, int $carePlanId, string $operacionSolicitada): void
    {
        ...
        if ($operacionSolicitada === SubmitConsultasSeguimientoIntake::MEDICACION_OP_RENOVACION
            && ($dup['block_renovacion_si_ajuste_abierto'] ?? true) === true
            && $opAbierta === SubmitConsultasSeguimientoIntake::MEDICACION_OP_AJUSTE) {
        ...
        if ($opAbierta === SubmitConsultasSeguimientoIntake::MEDICACION_OP_RENOVACION) {
            $msg = $catalog->duplicateMessage('open_renovacion');
        } elseif ($opAbierta === SubmitConsultasSeguimientoIntake::MEDICACION_OP_AJUSTE) {
