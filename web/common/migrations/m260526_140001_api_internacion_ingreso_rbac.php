...
/**
 * RBAC: ingreso a internación (UI JSON + API).
 */
class m260526_140001_api_internacion_ingreso_rbac extends Migration
{
    ...
    private const INHERIT_FROM = '/api/clinical/internacion/mapa-camas';
        ...
        foreach (self::ROUTES as $route) {
            if (!(new Query())->from($authItem)->where(['name' => $route])->exists($this->db)) {
                $this->db->createCommand()->insert($authItem, [
                    ...
                    'description' => 'API internación — ingreso',
                    'rule_name' => null,
                    'data' => null,
                    'created_at' => $now,
                    'updated_at' => $now,
