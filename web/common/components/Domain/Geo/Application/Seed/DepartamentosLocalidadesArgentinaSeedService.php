...
namespace common\components\Domain\Geo\Application\Seed;
...
final class DepartamentosLocalidadesArgentinaSeedService
{
    ...
    {
        $map = [];
        foreach ((new Query())->from('{{%geo_provincias}}')->select(['id_provincia', 'cod_indec'])->where(['id_pais' => (int) \common\models\Geo\Pais::requireByIso2('AR')->id_pais])->all() as $row) {
    ...
    private function assertLocalidadEnProvinciaPorCodIndec(int $idLocalidad, string $codIndec): void
    {
        ...
        $expected = Provincia::findOne([
            ...
            'id_pais' => (int) \common\models\Geo\Pais::requireByIso2('AR')->id_pais,
        ]);
