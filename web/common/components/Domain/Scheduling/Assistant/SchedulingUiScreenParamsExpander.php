...
final class SchedulingUiScreenParamsExpander implements UiScreenParamsExpanderInterface
{
    ...
    public static function expand(string $entity, string $action, array $params): array
    {
        ...
        $pesIdParam = (int) ($params['id_profesional_efector_servicio'] ?? 0);
        if ($pesIdParam > 0) {
            $params = self::syncParamsFromPes($params, $pesIdParam);
        }
    ...
    /**
     * El PES prevalece sobre draft.id_efector / id_servicio_asignado del flujo.
     *
     * @param array<string, mixed> $params
     * @return array<string, mixed>
     */
    private static function syncParamsFromPes(array $params, int $pesId): array
    {
