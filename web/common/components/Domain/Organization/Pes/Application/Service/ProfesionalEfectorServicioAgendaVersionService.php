...
use common\components\Domain\Scheduling\Agenda\Application\Service\TurnoIndisponibilidadImpactService;
...
final class ProfesionalEfectorServicioAgendaVersionService
{
    ...
    public static function previewImpacto(int $idPes, int $idEfector, array $post): array
    {
        ...
        $slotsAntes = $versionActual !== null
            ? AgendaSlotService::estimarSlotsPorDiaPromedio($versionActual, $intervaloActual)
            : 0;
        $slotsDespues = AgendaSlotService::estimarSlotsPorDiaPromedio($versionPropuesta, $intervaloNuevo);
        ...
        foreach ($turnosFuturos as $turno) {
            ...
            if (AgendaSlotService::horaEstaEnGrilla($versionPropuesta, $fecha, $hora, $intervaloNuevo)) {
                $alineados++;
                continue;
            }
            $vec = AgendaSlotService::vecinosEnGrilla($versionPropuesta, $fecha, $hora, $intervaloNuevo);
            $conflictos[] = [
