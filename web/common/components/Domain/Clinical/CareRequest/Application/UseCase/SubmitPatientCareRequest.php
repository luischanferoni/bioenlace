...
final class SubmitPatientCareRequest
{
    ...
    public function opcionesActoParaTriagePaso(): array
    {
        ...
        foreach ($this->actosReservables() as $acto) {
            ...
            $patientLabel = CareRequestMetadata::patientLabelForActo($code, $system);
    ...
    {
        return $this->catalog instanceof DbServiceLineActCatalog
            || $this->catalog instanceof CompositeServiceLineActCatalog;
    }
    ...
    public function aplicarFlagsEnDraft(array &$draft): void
    {
        ...
        if (trim((string) ($draft[self::DRAFT_MODO] ?? '')) === '') {
            $draft[self::DRAFT_MODO] = CareRequest::MODO_ESTUDIO;
        }
        ...
        $pedido = new CareRequest(
            ...
            $parsed['code'],
            $parsed['system'],
            $modo !== '' ? $modo : CareRequest::MODO_ESTUDIO,
            null,
            $efectorId,
            $parsed['display'] ?? null
        );
    ...
    public static function parseActoValue(string $raw): ?array
    {
        ...
        if (str_contains($raw, '|')) {
            ...
            $system = trim($system);
            $code = trim($code);
            if ($system === '' || $code === '' || !CodingSystems::isAllowed($system, CareRequestMetadata::allowedSystems())) {
                return null;
            }
        ...
        if (preg_match('/^\d{6,18}$/', $raw) === 1
            && CodingSystems::isAllowed(CodingSystems::SNOMED, CareRequestMetadata::allowedSystems())
        ) {
    ...
    {
        $norm = CareRequestMetadata::foldNlText($texto);
        ...
        foreach ($this->actosReservables() as $acto) {
            ...
            $keys = CareRequestMetadata::matchKeysForActo($code, $system, $display);
            foreach ($keys as $key) {
                if ($key === $norm || $code === trim($texto)) {
                    $exact[] = $row;
                    break;
                }
                if (CareRequestMetadata::nlTextHitsKey($texto, $key)) {
                    $partial[] = $row;
                    break;
                }
            }
    ...
    private function actosReservablesDesdeDb(): array
    {
        $q = (new \yii\db\Query())
            ...
            ->innerJoin(['la' => \common\models\Clinical\ServiceLineAct::tableName()], 'la.id_acto = a.id')
        ...
        foreach ($rows as $row) {
            ...
            if (!CodingSystems::isAllowed($system, CareRequestMetadata::allowedSystems())) {
                continue;
            }
            $out[] = [
