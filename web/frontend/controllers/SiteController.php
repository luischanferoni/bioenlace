...
class SiteController extends Controller
{
  ...
  /*  public function behaviors()
   ...
   /* public function beforeAction($action)
    ...
    /**
     * Devuelve los datos de inicio según encounter (turnos, internados o guardias) en JSON.
     * Usado por la vista inicio-turnos vía AJAX (misma estructura que API /api/v1/inicio/datos).
     */
    public function actionInicioDatos()
    {
        ...
        switch ($encounterClass) {
            case Consulta::ENCOUNTER_CLASS_AMB:
                ...
                return ['encounter_class' => $encounterClass, 'kind' => 'turnos', 'data' => $data, 'fecha' => $fecha];
            case Consulta::ENCOUNTER_CLASS_IMP:
                $data = InfraestructuraPiso::getInternadosPorEfector($idEfector);
                return ['encounter_class' => $encounterClass, 'kind' => 'internados', 'data' => $data, 'fecha' => $fecha];
            case Consulta::ENCOUNTER_CLASS_EMER:
                $data = $this->formatearGuardiasParaInicio($idEfector);
                return ['encounter_class' => $encounterClass, 'kind' => 'guardias', 'data' => $data, 'fecha' => $fecha];
            default:
                return ['encounter_class' => $encounterClass, 'kind' => null, 'data' => [], 'fecha' => $fecha];
        }
    }
