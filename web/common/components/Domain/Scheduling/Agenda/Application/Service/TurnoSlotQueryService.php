...
class TurnoSlotQueryService
{
    ...
    public static function findAvailableSlots(array $criteria, $limit = 10): array
    {
        ...
        if (!$idServicio) {
            throw new \InvalidArgumentException('TurnoSlotQueryService requiere id_servicio');
        }
        ...
        for ($offset = 0; $offset < $maxDias && count($out) < $limit; $offset++) {
            ...
            foreach ($agendasPorPes as $agenda) {
                ...
                $slots = AgendaSlotService::slotsParaDia($agendaLike, $dia, $intervalo);
                if ($slots === []) {
                    continue;
                }
    ...
    public static function findAvailableSlotsHubTeleconsulta(array $criteria, $limit = 10): array
    {
        ...
        if (!is_array($idServicios) || $idServicios === []) {
            throw new \InvalidArgumentException('TurnoSlotQueryService hub teleconsulta requiere id_servicios');
        }
