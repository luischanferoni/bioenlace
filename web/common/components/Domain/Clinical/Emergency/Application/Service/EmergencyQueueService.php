...
final class EmergencyQueueService
{
    ...
    public function tablero(int $idEfector, array $filters = []): array
    {
        ...
        $rows = $query->all();
        usort($rows, function (EmergencyEpisode $a, EmergencyEpisode $b): int {
    ...
    private function baseActiveQuery(int $idEfector, array $filters): ActiveQuery
    {
        ...
        if ($soloActivos || ($filters['solo_activos'] ?? false)) {
            $query->andWhere(['<>', 'g.estado', EmergencyEpisode::ESTADO_FINALIZADA]);
            $query->andWhere([
                'or',
                ['g.circuito_estado' => null],
                ['<>', 'g.circuito_estado', BoardState::FINALIZADO],
            ]);
        }
        ...
        if (!empty($filters['sin_triage'])) {
            $query->leftJoin(['gt' => EmergencyTriage::tableName()], 'gt.guardia_id = g.id')
                ->andWhere(['gt.id' => null]);
        }
     ...
     */
    private function serializeBoardRow(EmergencyEpisode $guardia): array
    {
        ...
        $triage = EmergencyTriage::findOne(['guardia_id' => (int) $guardia->id]);
        ...
        $row = [
            ...
            'circuito_estado_label' => BoardState::label($circuito),
    ...
    private function ingresoAt(EmergencyEpisode $guardia): string
    {
        return (string) ($guardia->ingreso_at
            ?: ($guardia->created_at ?: ($guardia->fecha . ' ' . ($guardia->hora ?? '00:00:00'))));
    }

    private function prioridadTriageForSort(EmergencyEpisode $guardia): ?int
    {
     ...
     */
    private function serializeClinicalBoardCounts(EmergencyEpisode $guardia): ?array
    {
