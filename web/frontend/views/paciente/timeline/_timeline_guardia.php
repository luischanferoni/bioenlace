...
switch ($historia['tipo']) {

    case 'pendiente':
        $estadoGuardia = 'ACTIVA';
        $bg = 'bg-soft-warning';
        break;
    case 'finalizada':
        $estadoGuardia = 'FINALIZADA';
        $bg = 'bg-soft-secondary';
        break;

    case 'atendida':
        $estadoGuardia = 'ATENDIDA';
        $bg = 'bg-soft-success';
        break;
} ?>

<p class="float-left mb-1 mt-1">
    Estado: <span class="badge <?= $bg ?>"><?= $estadoGuardia ?></span>
</p>
...
<div class="iq-media-group iq-media-group-1 timeline_footer mt-2 pb-3 border-bottom">
    ...
    <?= Guardia::footerTimeline($historia['tipo'], $historia['id'], $persona->id_persona) ?>
