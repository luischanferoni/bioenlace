    ...
    <div class="card-body">
          ...
          if (is_array($model->turnos) && count($model->turnos)>0 ) {
            ...
            foreach ($model->turnos as $key => $turno) {
              ...
              echo '</td>';
              echo '<td>';
              $prof = $turno->getProfesionalPersonaParaDisplay();
              echo $prof ? $prof->getNombreCompleto(Persona::FORMATO_NOMBRE_A_N) : 'Sin asignar';
              echo '</td>';
              echo '<td>';
              echo $turno->getNombreServicioParaDisplay();
              echo '</td>';
              echo '<td>';
              ...
              if (isset($turno->userAlta)) {
                ...
                echo $turno->rrhh->persona->getNombreCompleto(Persona::FORMATO_NOMBRE_A_N);
              }
              echo '</td>';
              /* echo '<td>';
