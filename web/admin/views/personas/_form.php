        ...
        // funcion para validar DNI y PUCO
         $("#persona-documento").blur(function(){
             ...
             $.ajax({
                    ...
                    data:{ dni:$("#persona-documento").val(),nombre:$("#persona-nombre").val()},
...
<div class="panel panel-default">
  <div class="panel-body">    
    ...
    <div class="row">
        ...
        <div class="col-sm-2">
            <?php
                if ($model->isNewRecord && !isset($_POST["Persona"]["documento_propio"])) {$model->documento_propio = 1;}
                echo $form->field($model, 'documento_propio', [
                    'template' => '{input}{error}{hint}'])->radioList([ '1' => 'Si', '0' => 'No'], ['prompt' => '']);

            ?>
        </div>        
        <?=
        Html::activeLabel($model, 'sexo', [
            'label' => 'Sexo: ',
            'class' => 'col-sm-1 control-label'
        ])
        ?>
        <div class="col-sm-2">
            <?php
                if ($model->isNewRecord) { 
                            $model->sexo = (isset($_POST["Persona"]["sexo"] ))? $_POST["Persona"]["sexo"] :0;
                            echo $form->field($model, 'sexo', [
                                'template' => '{input}{error}{hint}'])->radioList([ 'M' => 'Masculino', 'F' => 'Femenino', 'I' => 'Indeterminado',], ['prompt' => '']);
                }else {
                            echo $form->field($model, 'sexo', [
                                'template' => '{input}{error}{hint}'])->radioList([ 'M' => 'Masculino', 'F' => 'Femenino', 'I' => 'Indeterminado',], ['prompt' => '']);
                }
    ...
    <div class="row">
        ...
        <div class="col-sm-2">
            ...
            $form->field($model, 'fecha_defuncion', [
                'template' => '{input}{error}{hint}'])->widget(\yii\jui\DatePicker::className(), ['dateFormat' => 'dd/MM/yyyy']);
            ?>
        </div>
        <?php }?>
        <?=
       ...
       <div class="row">
            ...
            <div class="col-sm-2">
                ...
                $form->field($model_persona_telefono, 'numero', [
                    'template' => '{input}{error}{hint}'])->textInput(['maxlength' => true,'value'=> (isset($_POST["PersonaTelefono"]["numero"]))?$_POST["PersonaTelefono"]["numero"]:'',])
                ?>
            </div>
        </div> 
    <!-- --------------------DATOS EMAIL --------------------------*-->
        ...
        <div class="row">
            ...
            <div class="col-sm-3">
                ...
                echo $form->field($model_provincia, 'id_provincia', [
                    'template' => '{input}{error}{hint}'])->dropDownList($provincia, [ 'id' => 'id_provincia',
...
//.'/bioenlace/web/index.php?r=personas_telefono/update&id=' . $tells['id_persona_telefono'] . '&idp=' . $model->id_persona                            
                            ...
                            . 'href="'.Url::toRoute(['personas-telefono/update', 'id' => $tells['id_persona_telefono'], 'idp' => $model->id_persona]). '">'
                            . '<span class="glyphicon glyphicon-pencil"></span></a> <br>';
                        }                        
                        ?>
                        <a href="<?php echo Url::toRoute(['personas_telefono/create', 'idp' => $model->id_persona]);?>" class="btn btn-success">
        ...
        <table class="table table-striped table-bordered detail-view">
            <tbody>
                <tr><th style="width: 100px">EMAILS</th><td>
                        <?php
                    foreach ($mailsxpersona as $email) {
                        
                        echo $email['mail'];
                         echo ' - <a data-pjax="0" aria-label="Actualizar" title="Actualizar" '
                            . 'href="'.Url::toRoute(['personas_mails/update', 'id' => $email['id_persona_mail'], 'idp' => $model->id_persona]). '">'
                            . '<span class="glyphicon glyphicon-pencil"></span></a> <br>';
                     
                    }
                    ?>
                        <a href="<?php echo Url::toRoute(['personas_mails/create', 'idp' => $model->id_persona]);?>" class="btn btn-success">                        
...
//        }
    ...
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? 'Agregar' : 'Actualizar', ['class' => $model->isNewRecord ? 'btn btn-success pull-right' : 'btn btn-primary pull-right']) ?>
    </div>
