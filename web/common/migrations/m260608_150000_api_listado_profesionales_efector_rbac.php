...
/**
 * RBAC: listados staff por métrica (/api/listar).
 */
class m260608_150000_api_listado_profesionales_efector_rbac extends Migration
{
    private const ROUTE_TYPE = 3;

    private const ROUTE = '/api/listar';

    private const PARENT = '/api/info';

    public function safeUp()
    {
        if (!in_array($this->db->driverName, ['mysql', 'mysqli'], true)) {
            echo "m260608_150000: omitido (driver {$this->db->driverName}).\n";
        ...
        if ($this->db->schema->getTableSchema($authItem, true) === null) {
            echo "m260608_150000: sin tabla auth_item, omitido.\n";
        ...
        if ($this->db->schema->getTableSchema($childTable, true) === null) {
            echo "m260608_150000: sin tabla auth_item_child, omitido.\n";
    ...
    private function ensureRoute(string $authItem, string $name, int $now): void
    {
        ...
        $row = [
            ...
            'description' => 'API listados staff por métrica (DataAccess /listar)',
            'rule_name' => null,
            'data' => null,
            'created_at' => $now,
            'updated_at' => $now,
