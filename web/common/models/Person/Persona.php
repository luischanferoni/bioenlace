...
class Persona extends \yii\db\ActiveRecord
{
    ...
    */
    ...
    public function attributeLabels()
    {
        return [
            ...
            'fecha_defuncion' => 'Fecha DefunciÃ³n',
   ...
   /**
    ...
    /**
     * Turno pendiente del paciente para el servicio o el vÃ­nculo PES indicados.
     *
    ...
    //Calcula la edad en aÃ±os, meses y dÃ­as
    public function getEdadBebe()
    {
        ...
        $anios = $Y1 - $Y; //Calculo aÃ±os
        $meses = $m1 - $m; //Calculo meses
        $dias = $d1 - $d; //Calculo dÃ­as
        ...
        if ($anios !== 0) {
            $edad_bebe = $anios . '  aÃ±os,  ' . $meses . '  meses,  ' . $dias . '  dÃ­as  ';
        } else {
            $edad_bebe =  $meses . '  meses,  ' . $dias . '  dÃ­as  ';
        }
        return $edad_bebe;
    }
    ...
    /** @deprecated use {@see \common\components\Domain\Geo\Application\Service\GeografiaDepdropService::departamentosPorProvincia} */
    public function getDepartamentoxidprovincia($id)
    {
        return \common\components\Domain\Geo\Application\Service\GeografiaDepdropService::departamentosPorProvincia((int) $id);
    }

    // funcion para consultar las localidades segun el id de departamento
    /** @deprecated use {@see \common\components\Domain\Geo\Application\Service\GeografiaDepdropService::localidadesPorDepartamento} */
    public function getLocalidadxiddepartamento($idd)
    {
        return \common\components\Domain\Geo\Application\Service\GeografiaDepdropService::localidadesPorDepartamento((int) $idd);
    }
    ...
    {
        $value = \common\components\Domain\Person\Identity\Domain\Policy\CuilPolicy::normalize((string) ($this->$attribute ?? ''));
        if ($value === '') {
            $this->$attribute = null;

            return;
        }
        if (!\common\components\Domain\Person\Identity\Domain\Policy\CuilPolicy::isValid($value)) {
    ...
    public function documentoUnico($attribute, $params)
    {
        ...
        if ($this->documento_propio == 1) {
            ...
            if ($data['cnt'] == 3) {
                if ($this->isNewRecord) {
                    $this->addError($attribute, 'LÃ­mite alcanzado como documento ajeno (2).');
                } else {
                    if ($this->$attribute != $this->oldAttributes['documento']) {
                        $this->addError($attribute, 'LÃ­mite alcanzado como documento ajeno (2).');
                    }
...
//        echo $command->sql;
    ...
    /*
* MÃ©todos creados para obtener el evento activo para el paciente
* asÃ­ poder generar la url correspondiente
*/
    ...
    /**
     * Turno del dÃ­a pendiente alineado con servicio, efector y {@see Turno::$id_profesional_efector_servicio} (PES).
     *
     * @param int|string|null $idServicio
     * @param int|string|null $idProfesionalEfectorServicio PES a comparar con el turno (p. ej. el de la sesiÃ³n).
     * @param int|string|null $idEfector
     * @param int|string|null $idPesOverride si no es null/vacÃ­o, sustituye el PES de sesiÃ³n para la rama `$matchPes`; si es null, se usa `getIdProfesionalEfectorServicio()` (contexto staff autenticado).
     */
