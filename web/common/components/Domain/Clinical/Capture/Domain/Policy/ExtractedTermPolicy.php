...
/**
 * Policy: ¿un término extraído por IA es clínicamente plausible?
 * Terminología vía {@see TerminologyLookupPort} (sin acoplar a Platform ni adapters).
 */
final class ExtractedTermPolicy
{
    private TerminologyLookupPort $terminologyLookup;

    public function __construct(TerminologyLookupPort $terminologyLookup)
    {
        $this->terminologyLookup = $terminologyLookup;
    }
    ...
    public function isPlausibleIsolatedDiagnosisCandidate($item, string $clinicalText, array $config = []): bool
    {
        ...
        if (ExtractionPostProcessPolicy::textMatchesClinicalLexiconPattern($term, 'subjective_complaint')) {
            return false;
        }

        if (ExtractionPostProcessPolicy::textMatchesClinicalLexiconPattern($clinicalText, 'narrative_framing')) {
            return false;
        }
    ...
    private function matchesRetainLexicon(string $term, string $clinicalText, array $config): bool
    {
        ...
        foreach ($retainKeys as $key) {
            ...
            $lexiconKey = trim($key);
            if (ExtractionPostProcessPolicy::textMatchesClinicalLexiconPattern($clinicalText, $lexiconKey)) {
                return true;
            }
            if ($term !== '' && ExtractionPostProcessPolicy::textMatchesClinicalLexiconPattern($term, $lexiconKey)) {
                return true;
            }
        }
