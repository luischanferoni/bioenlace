 ...
 */
final class EmergencySlaService
{
     ...
     * }
     */
    public function evaluate(EmergencyEpisode $guardia, int $minutosEspera, ?string $circuitoEstado, ?int $prioridadTriage): array
    {
        $config = EfectorEmergencyConfig::forEfector((int) $guardia->id_efector);
        $sinTriage = $circuitoEstado === BoardState::ESPERA_TRIAGE
        ...
        if (in_array($circuitoEstado, [BoardState::EN_ATENCION, BoardState::FINALIZADO, BoardState::DERIVADO], true)) {
            return [
                'sla_violado' => false,
                'sla_tipo' => null,
