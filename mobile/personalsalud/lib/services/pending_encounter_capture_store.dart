...
/// Persistencia local de capturas clínicas pendientes (texto + audio).
///
/// Estructura:
/// ```
/// <appDoc>/pending-encounters/
///   index.json
///   <id>.m4a
/// ```
class PendingEncounterCaptureStore {
      ...
      try {
        ...
        if (raw is List) {
          for (final row in raw) {
            if (row is Map) {
              final item =
                  PendingEncounterCapture.fromJson(Map<String, dynamic>.from(row));
        ...
        // Índice corrupto: partir vacío; no borrar audios huérfanos aquí.
      }
    }
    _loaded = true;
  }
  ...
  /// Copia un audio temporal al store durable; devuelve el nombre de archivo.
