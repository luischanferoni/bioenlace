...
Definir el shape de `ui_json` (blocks, fields, list, submit) para `ui_definition` y `ui_submit_result`.

## Actores

- Backend (`UiDefinitionTemplateManager`, `UiScreenService`).
- Clientes web/Flutter que renderizan blocks.

## Anclas

| Área | Ruta |
|------|------|
| GET view | `/api/v1/<entidad>/<accion>`, `/api/v1/clinical/...` |
| Custom widget | [VIEWS_JSON_CUSTOM_WIDGET.md](./VIEWS_JSON_CUSTOM_WIDGET.md) |
