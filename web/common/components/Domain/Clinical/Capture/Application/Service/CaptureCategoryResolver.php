 ...
 */
final class CaptureCategoryResolver
{
    ...
    public function resolve(?EncounterDefinition $definition, array $body = [], ?string $actorOverride = null): array
    {
        ...
        $actor = $actorOverride !== null && $actorOverride !== ''
            ? CaptureActorCatalog::normalize($actorOverride)
    ...
    public function resolveActorItemName(): string
    {
        ...
        if ($idServicio > 0) {
            ...
            if ($servicio !== null) {
                return CaptureActorCatalog::normalize((string) $servicio->item_name);
            }
        }
        try {
            if (User::hasRole(['enfermeria'], false)) {
                return CaptureActorCatalog::ACTOR_ENFERMERIA;
            }
            if (User::hasRole(['Medico'], false)) {
                return CaptureActorCatalog::ACTOR_MEDICO;
            }
        } catch (\Throwable $e) {
            return '';
        }
    ...
    {
        foreach (CaptureActorCatalog::extraSteps($actor, $encounterClass) as $extra) {
            if ($this->indexOfModelo($categorias, $extra['modelo']) === null) {
                $categorias = $this->insertAfterTitle($categorias, $extra, null);
            }
        }
        foreach (CaptureActorCatalog::suggestedModelos($actor, $encounterClass) as $modelo) {
