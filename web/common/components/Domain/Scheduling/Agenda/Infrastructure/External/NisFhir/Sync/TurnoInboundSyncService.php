...
use common\components\Domain\Scheduling\Agenda\Infrastructure\External\NisFhir\Contract\FhirSchedulingInboundConnector;
...
final class TurnoInboundSyncService
{
    ...
    private function syncSlotClaimForInbound(Turno $turno, ?string $beforeEstado, bool $isNew): void
    {
        ...
        if ($isNew || $beforeEstado === Turno::ESTADO_CANCELADO) {
            ClaimTurnoSlot::tryClaim($idPes, $fecha, $hora, $idTurno);
            return;
        }
        ClaimTurnoSlot::moveClaim($idTurno, $idPes, $fecha, $hora);
    }

    private function applyDtoToTurno(
        Turno $turno,
        FhirAppointmentInbound $dto,
        string $scheduleId,
        ?string $trust,
        ?int $idPes
    ): void {
