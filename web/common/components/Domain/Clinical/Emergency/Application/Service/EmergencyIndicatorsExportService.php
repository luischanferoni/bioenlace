 ...
 */
final class EmergencyIndicatorsExportService
{
    /**
    ...
    public function buildCsv(int $idEfector, ?string $fechaDesde = null, ?string $fechaHasta = null): array
    {
        ...
        if ($hasta >= date('Y-m-d')) {
            $live = (new EmergencyIndicatorsService())->resumen($idEfector);
            $slaCount = (new EmergencyIndicatorsService())->contarSlaIncumplidosTablero($idEfector);
            $rows[] = [
                date('Y-m-d'),
