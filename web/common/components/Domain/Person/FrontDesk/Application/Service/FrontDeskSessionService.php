...
namespace common\components\Domain\Person\FrontDesk\Application\Service;
...
final class FrontDeskSessionService
{
    ...
    public function iniciar(array $body, int $idEfector): array
    {
        ...
        $row->started_at = $now;
        $row->expires_at = date('Y-m-d H:i:s', time() + FrontDeskSessionConfigService::ttlMinutes() * 60);
        $row->created_at = $now;
        if (!$row->save()) {
    ...
    private function sesionAbiertaActual(): ?FrontDeskSession
    {
        $staffUserId = (int) Yii::$app->user->id;
        if ($staffUserId <= 0) {
            return null;
        ...
        $now = date('Y-m-d H:i:s');
        /** @var FrontDeskSession|null $row */
        $row = FrontDeskSession::find()
    ...
    private function identityMethod(array $body): string
    {
        ...
        return FrontDeskSession::METHOD_ID_PERSONA;
    }
     ...
     * @return array<string, mixed>
     */
    private function serialize(FrontDeskSession $row): array
    {
