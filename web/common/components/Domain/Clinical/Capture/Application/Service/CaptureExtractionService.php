...
{
    public function extract(array $body): array
    {
        try {
            [$idProfesionalEfectorServicio, $idServicio] = array_slice(
                OperationalContextResolver::resolve($body),
                0,
                2
            );
            ...
            $resultadoFormato = CaptureTextService::prepararParaIAConFormato(
                $textoConsulta,
                $servicio->nombre,
                $tabId,
            ...
            if (trim($textoLimpio) !== trim($textoConsulta)) {
                $logger->registrar(
                    ...
                    null,
                    $textoLimpio,
                    ['metodo' => 'CaptureTextService::limpiarTexto']
                );
            }

            $categorias = $this->categoriesForConfig($idConfiguracion, $body);

            $resultadoIA = $this->extractFromProcessedText(
                $textoLimpio,
                $servicio->nombre,
                $categorias,
                PatientAiContextBuilder::resolveSubjectPersonaIdFromBody($body),
            ...
            $logger->registrarJsonIa($datos, 'CaptureExtractionService::extractFromProcessedText');
            ...
            $resultado = [
                ...
                'stt_provenance' => $speech['provenance'] ?? CaptureSpeechInputResolver::PROVENANCE_TEXT_ONLY,
            ...
            $analysisCacheToken = CaptureAnalysisCache::store(
                $cacheBody,
                $extraidosForCache,
            ...
            if ($textoConsulta !== '' && $textoConsulta !== $textoProcesado) {
                CaptureAnalysisCache::store($cacheBody, $extraidosForCache, $textoConsulta);
            }
            if ($analysisCacheToken !== null) {
                ...
                ], ['metodo' => 'CaptureAnalysisCache::store']);
            }
            ...
            Yii::error('Error en CaptureExtractionService::extract: ' . $e->getMessage() . "\n" . $e->getTraceAsString(), 'consulta-ia');
    ...
    public function extractFromProcessedText(
        $texto,
        $servicio,
        $categorias,
        ...
        try {
            ...
            if ($resultado && !isset($resultado['error'])) {
                ...
                return (new ExtractionPostProcessService())->apply(
                    $normalizado,
                    is_array($categorias) ? $categorias : [],
                    (string) $texto
                );
            ...
            Yii::error('Error en extractFromProcessedText: ' . $e->getMessage() . "\n" . $e->getTraceAsString(), 'consulta-ia');
            return [
                'datosExtraidos' => [
                    'Error' => [
     ...
     */
    public function categoriesForConfig($idConfiguracion, array $body = [])
    {
        ...
        return (new \common\components\Domain\Clinical\Capture\Application\Service\CaptureCategoryResolver())
