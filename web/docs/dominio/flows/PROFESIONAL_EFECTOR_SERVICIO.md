...
Alta vía API `configurar-agenda` / `preview-configurar-agenda` (`ProfesionalEfectorServicioAgendaVersionService`). Detalle: [turnos/flows/agenda-intervalo-y-reservas.md](../../turnos/flows/agenda-intervalo-y-reservas.md).
...
2. **Consumidores adicionales**: `m260508_000003_consumidores_id_profesional_efector_servicio` (`consultas`, `consultas_derivaciones`, `documentos_externos`, `guardia`). `m260508_000004_consumidores_pes_lote2` (atenciones enfermería, suministro medicación por consulta, internación, encuesta parches, programas/dispensa diabetes, autofacturación SUMAR, `abreviaturas_rrhh`, y **si existe** la tabla `seg_nivel_internacion_practica`, columnas PES `_solicita` / `_realiza`). Donde solo hay `id_rr_hh`, el backfill usa el primer `rrhh_servicio` del RRHH (MIN id). Para crear la tabla en producción: `m260508_000005_create_seg_nivel_internacion_practica` (requiere `seg_nivel_internacion`). Pendiente p. ej. `solicitud_rrhh` si se define semántica PES por servicio.
