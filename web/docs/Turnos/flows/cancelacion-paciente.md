...
- Paciente (app / asistente `turnos.cancelar-como-paciente-flow`).
- Staff en calendario web (mismo motivo cuando cancela en nombre del paciente).

## Anclas

| Canal | Método / ruta |
|-------|----------------|
| API paciente | `TurnosController::actionCancelarComoPaciente` |
| Web calendario | `TurnosController::actionDelete` (POST `turnos/delete`) |
| Intent | `turnos.cancelar-como-paciente-flow` |
| Modelo | `Turno::ESTADO_MOTIVO_CANCELADO_PACIENTE` |
