...
Documentar el modelo **greenfield** de grilla (versiones, intervalo fijo, solapamiento, preview de cambios y conflictos de agenda).

## Actores

- Administrador de agenda (staff PES).
- Paciente al reprogramar o al detectar conflicto tras cambio de grilla.

## Secuencia (publicar versión)

1. Staff configura intervalo y bloques en versión nueva con `vigente_desde`.
2. Si hay turnos afectados → preview obligatorio (`confirmar_cambios=1`).
3. Publicación actualiza espejo `profesional_efector_servicio_agenda` y puede encolar `turno_agenda_conflicto`.

## Anclas

| Paso | Componente / tabla |
|------|---------------------|
| Intervalos permitidos | `AgendaIntervaloMinutos` |
| Versiones | `profesional_efector_servicio_agenda_version` |
| Migración | `m260516_000001_agenda_intervalo_versioning` |
| API conflictos | rutas `resolver-conflicto-agenda-como-paciente` |
