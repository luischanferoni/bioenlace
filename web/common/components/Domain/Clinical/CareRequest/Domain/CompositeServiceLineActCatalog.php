...
use common\components\Domain\Clinical\CareRequest\Infrastructure\External\SnowstormActEclMembership;

/**
 * Une puente explícito (`linea_acto`) + capacidad ECL.
 * Preferente del puente gana; dedupe por id_servicio / code.
 */
final class CompositeServiceLineActCatalog implements ServiceLineActCatalogInterface
{
    private ServiceLineActCatalogInterface $explicit;
    private EclCapacityCatalog $ecl;

    public function __construct(
        ?ServiceLineActCatalogInterface $explicit = null,
        ?EclCapacityCatalog $ecl = null
    ) {
        $this->explicit = $explicit ?? new DbServiceLineActCatalog();
        $this->ecl = $ecl ?? new EclCapacityCatalog();
    }

    public static function defaultCatalog(): self
    {
        return new self(new DbServiceLineActCatalog(), new EclCapacityCatalog(new SnowstormActEclMembership()));
    }
