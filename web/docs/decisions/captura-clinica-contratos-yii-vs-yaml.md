...
- Nuevas tipologías de captura: agregar `*Input` + `completenessForExtractedRow` en el `Consulta*`; el validator lo descubre por nombre de modelo. No alargar listas planas ni if-chains.
...
- Path legacy sin `modelo` en categorías: sigue interpretando `campos_requeridos` como lista plana (compat tests / datos viejos).
