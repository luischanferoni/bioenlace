...
- [x] `POST asignar` + `POST derivar` + `POST finalizar` (`EmergencyOperationService`)
- [x] Push servidor: `EMERGENCY_ASSIGNED_TO_YOU`, `EMERGENCY_PATIENT_CRITICAL` (`EmergencyPushService`)
...
- [x] Deprecar `EmergencyEpisode::footerTimeline` hacia inicio EMER
