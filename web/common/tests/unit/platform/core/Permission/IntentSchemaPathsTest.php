...
namespace common\tests\unit\platform\core\Permission;
...
class IntentSchemaPathsTest extends Unit
{
    ...
    public function testDomainComesFromFolder(): void
    {
        IntentSchemaPaths::resetIndexCache();
        $this->assertSame('scheduling', IntentSchemaPaths::domainForIntentId('turnos.crear-como-paciente'));
        $this->assertSame('organization', IntentSchemaPaths::domainForIntentId('profesionales.conteo-efector'));
        $this->assertSame('platform', IntentSchemaPaths::domainForIntentId('data-access.info'));
        $this->assertSame('clinical', IntentSchemaPaths::domainForIntentId('urgencias.triage-paciente-guardia'));
    }
}
