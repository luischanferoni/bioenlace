...
class ParameterExtractor
{
    ...
    private static function extractParameter($message, $messageLower, $param, $intent)
    {
        switch ($param) {
            case 'id_persona':
                return self::extractIdPersona($messageLower);

            case 'id_internacion':
                return self::extractIdInternacion($messageLower);

            case 'id_cama':
                return self::extractIdCama($messageLower);
    ...
    private static function extractIdPersona($messageLower)
    {
        // Ej: "paciente 123", "id_persona: 123", "persona=123"
        $patterns = [
            '/\b(id_persona|persona|paciente)\s*[:=]?\s*(\d+)\b/i',
        ];
        foreach ($patterns as $pattern) {
            if (preg_match($pattern, $messageLower, $m)) {
                return (int)$m[2];
            }
        }
        return null;
    ...
    private static function extractIdInternacion($messageLower)
    {
        // Ej: "internacion 55", "id_internacion:55", "internación #55"
        $patterns = [
            '/\b(id_internacion|internaci[oó]n|internacion)\s*[:#=]?\s*(\d+)\b/i',
        ];
        foreach ($patterns as $pattern) {
            if (preg_match($pattern, $messageLower, $m)) {
                return (int)$m[2];
    ...
    private static function extractIdCama($messageLower)
    {
        // Ej: "cama 12", "id_cama: 12"
        $patterns = [
            '/\b(id_cama|cama)\s*[:#=]?\s*(\d+)\b/i',
        ];
        foreach ($patterns as $pattern) {
            if (preg_match($pattern, $messageLower, $m)) {
                return (int)$m[2];
