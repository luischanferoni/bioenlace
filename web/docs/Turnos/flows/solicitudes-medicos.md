...
Intercambio de mensajes/solicitudes entre profesionales del efector según modo configurado (directo, intermediario, auto-asignación).

## Actores

- Profesional solicitante y destinatario (según modo).
- Administrador del efector (listado en config).

## Anclas

| Paso | Ruta / tabla |
|------|----------------|
| API listar/crear | `SolicitudProfesionalController` — `GET|POST /api/v1/solicitud-profesional` |
| Persistencia | `solicitud_rrhh` |
| Config modo | `efector_turnos_config.modo_comunicacion_medicos` |
