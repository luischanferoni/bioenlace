...
- Configuración: alertas, recordatorios de planes de tratamiento, **resumen de historia en el asistente** (extracto acotado al chat; se puede apagar), preferencia de aviso cuando un representante actúa (N9). Privacidad e IA: [ia-datos-y-privacidad.md](./ia-datos-y-privacidad.md).
...
- **Captura clínica** unificada: timeline del paciente + formulario encounter (texto/audio → API `clinical/encounter/*`); muta por encounter, rol y **servicio del centro** — [captura-clinica.md](./captura-clinica.md), [glosario-servicio-pes-acto.md](./glosario-servicio-pes-acto.md). En ambulatorio, el timeline muestra **motivos pre-turno** (intake, resumen del chat, cohorte) antes del dictado — [recorrido-pre-post-consulta.md](./recorrido-pre-post-consulta.md).
