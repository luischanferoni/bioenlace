...
use common\components\Domain\Clinical\Capture\Application\Service\ResolutionApplier;
use common\models\Clinical\Input\IndicacionInput;
use common\models\Clinical\ConsultaIndicaciones;

class CaptureResolutionApplierTest extends Unit
{
    public function testFactoryMakeAndParse(): void
    {
        $issue = CaptureIssueFactory::make(
            'Indicaciones',
            0,
            'Plazo dias',
            [
        ...
        $emptyOpts = CaptureIssueFactory::make('X', 0, 'Campo', [], false);
        $this->assertFalse($emptyOpts['allow_custom']);
        $this->assertSame([], $emptyOpts['options']);

        $parsed = CaptureIssueFactory::parseIssueId($issue['id']);
    ...
    public function testApplierWritesPlazoAndClearsIssue(): void
    {
        ...
        $updated = (new ResolutionApplier())->apply(
            $extraidos,
