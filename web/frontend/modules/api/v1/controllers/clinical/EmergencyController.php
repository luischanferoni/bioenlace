 ...
 *
 * Id público RBAC/URL: `emergency-guardia` (alias en Module controllerMap).
 *
...
class EmergencyController extends BaseController
{
    ...
    public function actionAsignar(int $guardiaId): array
    {
        try {
            ...
            if ($pesId <= 0) {
                $resolved = EmergencyEfectorAccess::resolvePesId(null);
                if ($resolved === null) {
    ...
    public function actionSlaConfig(): array
    {
        try {
            $idEfector = $this->requireGuardiaEfector();
            $data = (new EmergencySlaService())->configForEfector($idEfector);
        } catch (\InvalidArgumentException $e) {
            return $this->error($e->getMessage(), null, 400);
        } catch (ForbiddenHttpException $e) {
            return $this->error($e->getMessage(), null, 403);
