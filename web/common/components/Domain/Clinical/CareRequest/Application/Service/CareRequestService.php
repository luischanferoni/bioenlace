...
final class CareRequestService
{
    ...
    public function __construct(?ServiceLineActCatalogInterface $catalog = null)
    {
     ...
     */
    public function missingSlots(CareRequest $pedido): array
    {
        return $this->resolve($pedido)['missing'];
    }

    public function isComplete(CareRequest $pedido): bool
    {
     ...
     */
    public function resolve(CareRequest $pedido): array
    {
        ...
        if ($pedido->hasLinea() && !$pedido->hasActo()) {
            ...
            if ($pedido->hasActoDisplayWithoutCode()) {
                ...
                $default = CareRequestMetadata::defaultActoForModo($pedido->modo);
                if ($default !== null) {
                    $fromPuente = $this->catalog->actosForLinea((int) $pedido->lineaId, $pedido->efectorId);
    ...
    private function normalizeActoSystem(CareRequest $pedido): CareRequest
    {
        if (!$pedido->hasActo()) {
        ...
        if (!CodingSystems::isAllowed((string) $pedido->actoSystem, CareRequestMetadata::allowedSystems())) {
