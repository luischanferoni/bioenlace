...
`GET /home/panel` sección `emergency_board` incluye `puede_triage` (`EmergencyBoardCapabilityService` + `home-panel-manifest.yaml` `triage_roles`). Flutter abre `EmergencyTriageScreen` si es true; si no, mensaje para médico.
