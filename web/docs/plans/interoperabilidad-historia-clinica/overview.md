...
El **único faltante deliberado** en Fase 1 es el **POST real** al endpoint nacional (URL, OAuth y perfiles finales dependen del contrato MSAL / provincial).
...
| **Saliente (batch)** | Reintento de jobs fallidos / conciliación | Cron `clinical-history-exchange/process-outbound` | 1 |
...
- **Auditoría** — staff con permiso API (futuro) consulta estado de jobs.
...
| **B — Cola dry-run** | Encola y mapea Bundle; conector `null` marca `SKIPPED`. Para QA del mapper. |
...
2. Cron procesa la cola; conector null → `SKIPPED` con mensaje claro.
