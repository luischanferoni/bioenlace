 ...
 */
final class InpatientEpicrisisTemplateAdminService
{
    /** @var list<string> */
    ...
    {
        $q = (new Query())
            ->from(['p' => InpatientEpicrisisTemplate::tableName()])
            ->where([
                'or',
    ...
    public function crear(array $payload, int $idEfector, bool $isSuperadmin): array
    {
        ...
        $model = new InpatientEpicrisisTemplate();
     ...
     * @param array<string, mixed> $payload
     */
    private function applyPayload(InpatientEpicrisisTemplate $model, array $payload, int $idEfector): void
    {
    ...
    private function assertCanManage(InpatientEpicrisisTemplate $model, int $idEfector, bool $isSuperadmin): void
    {
    ...
    {
        $row = (new Query())
            ->from(InpatientEpicrisisTemplate::tableName())
            ->where(['id' => $id])
            ->one();
    ...
    private function findModel(int $id): InpatientEpicrisisTemplate
    {
        $model = InpatientEpicrisisTemplate::findOne($id);
        if ($model === null) {
            throw new \InvalidArgumentException('Plantilla no encontrada.');
        }
    ...
    private function firstError(InpatientEpicrisisTemplate $model): string
    {
