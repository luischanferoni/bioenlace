...
Definir umbrales de cancelaciones por paciente que bloquean o advierten autogestión (cancelar, reprogramar, reservar) en la app.

## Actores

- Paciente (consulta política / intent bloqueado).
- Administrador del efector (config y liberaciones).

## Anclas

| Paso | Componente |
|------|------------|
| Config umbrales | `efector_turnos_config` |
| Liberación | `persona_efector_autogestion_liberacion` |
| API política | `TurnosController` — `politica-como-paciente` |
| Intent consulta | `turnos.consultar-politica-autogestion-flow` |
