...
| **Contrato slots (paciente/staff)** | `turnos/slots-disponibles-como-paciente`: `slot_id` canónico `pes:<id>\|fecha\|hora\|intervalo` (compat. 3 partes); grilla por versión de agenda y ocupación por solapamiento — ver [turnos/flows/agenda-intervalo-y-reservas.md](../../turnos/flows/agenda-intervalo-y-reservas.md). UI: dos pasos día → horario; `raw=1` → `por_dia` + `available_filters`. |
...
- `id_profesional_efector_servicio`, `slot_id` tipo `pes:<id>|<fecha>|<hora>|<intervalo>` (compat. sin intervalo), y `servicio`: `{ id_servicio, nombre }`.
